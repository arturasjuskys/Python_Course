...
#	JSON - dump
		#	this will convert Python object ot JSON string
import json
class Cat2:
	def __init__(self, name, breed):
		self.name = name
		self.breed = breed
c = Cat2("Charles", "Tabby")
...
print(frozen)
# {"py/object": "__main.Cat", "breed": "Tabbt", "name": "Charles"}

#	This will create a new .json with text from above example 'print(frozen)'
with open("cat.json", "w") as file:
	frozen = jsonpickle.encode(c)
	file.write(frozen)

#	This example will read cat.json file and will print it out
	...
	with open("users.csv", "w") as file:
		...
		for row in rows:
			if row[0] == old_first and row[1] == old_last:
				csv_writer.wroterow([new_first, new_last])
				count += 1
			else:
